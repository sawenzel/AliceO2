...
public:
  ...
  bool generateEvent() override { for(int i = 0; i < mMult; ++i) { mEvent.push_back(sampleParticle()); } return true; }
  bool importParticles() override { std::copy(mEvent.begin(), mEvent.end(), std::back_insert_iterator(mParticles)); return true; }
