...
struct GeneratorFileOrCmd {
 ...
 protected:
   ...
   *        own before killing its process group.  Where an intermediate
   *        shell survives - a wrapper script, rather than a command
   *        /bin/sh execs in place - the generator's CPU time reaches our
   *        RUSAGE_CHILDREN only once that shell has reaped it, which it
   *        can no longer do once we have killed it.  Zero (the default)
   *        kills immediately.
   *
  ...
  /**
   * Terminate the background command, if one was started, when the
   * generator is stopped normally: it has produced everything that was
   * asked of it by then, so it is given a grace period to exit by itself
   * (see @c terminateCmd) that only has to cover its teardown.
   */
  void stopCmd();
  /**
