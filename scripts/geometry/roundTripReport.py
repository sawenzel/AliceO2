...
One document, generated from a corpus on the fly, answering three questions that are usually
answered by reading four different JSON files at once:

  * **what went in** -- the source `TGeoShape`: its class, and for a boolean its recursion depth,
    leaf count and leaf classes, because a chain of 33 subtractions and a balanced tree of 33
    leaves are the same size and nothing like the same shape;
  * **what came out** -- which representation the cascade emitted, on what evidence
    (`dV_sym` against its acceptance band), and where it declined, in the recogniser's own words;
  * **whether the two are the same solid** -- the known-source verdict, which is the only test in
    the pipeline that ever sees the original `TGeoVolume`.

It also carries the **feature matrix**: every distinct source shape class in the corpus against
what the round trip made of it. That is the table that says what the pipeline supports, measured
over a real geometry rather than asserted, and it is the one to read first.

Nothing here re-derives anything. Every number is read from an instrument that already produced
it -- `csg_report.json` (the cascade), `knownsource.json` (`checkKnownSource.py`), the sidecars
themselves (`csg/planar.py` for tessellation exactness) -- and the source-shape description comes
from `exportSourceShapes.py`, which is where the writer-report resolution already lives. A field
this cannot read is reported as unknown and never as a zero.
...
from csg import planar  # noqa: E402
...
def source_descriptions(module, source_dir, conv_dir, refresh=False):
    ...
    Delegates to `exportSourceShapes.export_run(write=False)`, which is where the writer-report
    resolution lives -- duplicate names, `<name>__body` solids, mirrored prototypes. Importing ROOT
    costs a few seconds per module, so the answer is cached in `original_report.json` and reused.
    """
...
def read_module(module, source_dir, conv_dir, with_sources=True, refresh=False):
    ...
    for part in report.get("parts", []):
        ...
        # A conversion made before the converter recorded this simply has no field, and the answer
        # is in the sidecar either way -- so it is computed here rather than left as a gap, and an
        # older corpus reads the same as a fresh one.
    ...
    summary = {
        ...
        # The writer's own coverage. A volume it declined never reaches the converter at all, so a
        # report that counted only converted parts would call the pipeline complete while quietly
        # omitting whatever could not be exported in the first place.
...
# A "family" is a volume name with its trailing _<digits> groups stripped. It exists because some
# detector geometries name every instance of a component separately -- MFT has 5144 volumes called
# `capacitor_<i>_<j>_<k>` that are one 0.05 x 0.025 x 0.025 box -- and listing 19 177 rows of which
# 16 965 say the same thing is not a document. Grouping is presentation only; every count in this
# report is still over parts.
def family(name):
...
def redundancy(summaries):
    """Per module: how many solid-carrying volumes are geometric duplicates of another.

    Read from the writer report, whose per-volume rows carry the shape class and the capacity. Two
    volumes of one family with the same class and capacity are the same solid built twice, and a
    geometry that does that pays for it in every downstream stage -- the manager's volume table,
    the STEP file, this pipeline's per-part acceptance test, and navigation.
    """
    out = []
    for s in summaries:
...
def feature_matrix(rows):
    ...
    A `TGeoCompositeShape` is counted once as itself and once per *leaf class* it contains, in a
    second table, because "does the pipeline support TGeoXtru" is a different question from "does
    it support a boolean that contains one".
    """
...
def render_markdown(rows, summaries, per_part=True):
    ...
    doc.append(
        ...
        "triangulation is the same solid rather than an approximation of it (`csg/planar.py`). It "
