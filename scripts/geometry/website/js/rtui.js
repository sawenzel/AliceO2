...
import { Raytracer, VIEWS, VIEW_GROUPS, REP_VIEWS, DIFF_VIEWS, SUBJECT_DIFF_VIEWS,
         SUBJECT_VIEW, representationFor, REPRESENTATION_NOTE } from './raytrace.js';
import { REPRESENTATIONS, repLabel, repRole, partHas, partPath, partNote } from './representations.js';
...
export function initRaytracerTab() {
      ...
      <div>
        ...
        <div class="pane">
          ...
          <p class="muted small">The picture is produced by an engine that answers rays. The local engine is
            ...
            two can be subtracted. The bridge holds <strong>one</strong> shape at a time, so a switch between
            two representations re-loads the part's other file &mdash; the <code>surfaces.bin</code> sidecar, the
            <code>shape.root</code> composite, the <code>flatcsg.bin</code> halfspace solid or the
            <code>facets.bin</code> mesh &mdash; before the first band goes out. The service dispatches on the
            file's own magic, so all four are one endpoint.</p>
          <div class="row">
        ...
        <div class="pane">
          ...
          <p class="muted small">The same picture &mdash; the exact surfaces &mdash; asked of each engine in
            turn, and then the same part's <em>other representations</em> asked of the same kernel: the CSG
            composite, the flat halfspace solid and the tessellation. Each row is the last such frame, and the
            whole of it was that engine's work, so they are directly comparable when the camera and the
            resolution agree. A difference or parity frame is a different question and does not appear here.</p>
  ...
  // A bridge view of another representation asks nothing of the exact sidecar, so it stays open
  // on a part that has no surfaces_*.bin.
  const LOCAL_MESH_INDEPENDENT = new Set(['csgBridge', 'flatcsgBridge', 'meshBridge']);
  const GROUP_LABEL = {
    shaded: 'one representation, shaded',
    differences: 'two representations, subtracted',
    watertightness: 'crossing parity',
  };
  for (const group of VIEW_GROUPS) {
    const optgroup = document.createElement('optgroup');
    optgroup.label = GROUP_LABEL[group] || group;
    for (const view of VIEWS.filter(v => v.group === group)) {
  ...
  /// Why a representation cannot be traced for this part, named for the representation.
  function whyNotRepresentation(rep) {
    if (rep === 'shape') { return whyNotCSG(); }
    if (rep === 'flatcsg') {
      return 'the cascade did not emit a flat halfspace solid for this part, so there is no ' +
             'flatcsg_*.bin in testdata/ to trace';
    }
    if (rep === 'cellstree') {
      return 'this part has no flat-CSG decomposition to emit as a plain composite, so there is ' +
             'nothing to compare one against';
    }
    if (rep === 'original') {
      return 'this part came from CAD, not from an existing detector geometry, so there is no ' +
             'earlier TGeoShape it was made from -- run exportSourceShapes.py for a round-tripped part';
    }
    if (rep === 'mesh') { return 'no facets_*.bin was copied for this part'; }
    return 'no surfaces_*.bin for this part';
  }

  /// Turn the exact views off for a part with no sidecar, and every representation view off for a
  /// part that does not carry that representation's artefact -- in both cases saying why rather
  /// than failing.
  function applyCoverage() {
    const exact = !!state.sidecarBuffer;
    const part = state.part;
    for (const option of viewSelect.options) {
      const rep = representationFor(option.value);
      const needsRep = REP_VIEWS.has(option.value);
      const has = needsRep ? partHas(part, rep) : true;
      option.disabled = (!exact && !MESH_ONLY_VIEWS.has(option.value) && !LOCAL_MESH_INDEPENDENT.has(option.value))
        || (needsRep && !has);
      if (needsRep) {
        option.title = has ? `traces ${partPath(part, rep)} on the bridge` : whyNotRepresentation(rep);
      }
    }
    ...
    const notes = [];
    if (!exact) {
      notes.push(`${part ? part.name : 'This part'} is tessellated only -- no exact ` +
    ...
    const missing = REPRESENTATIONS.filter(r => r.key !== 'surface' && !partHas(part, r.key));
    for (const rep of missing) {
      notes.push(`No ${rep.label} artefact for this part: ${whyNotRepresentation(rep.key)}.`);
    }
    const carried = REPRESENTATIONS.filter(r => partHas(part, r.key));
    if (carried.length > 1) {
      notes.push(`This part carries ${carried.length} subjects -- ` +
        carried.map(r => r.label).join(', ') +
        ' -- and every one of them can be put on this camera through the bridge.');
      for (const rep of carried) {
        const note = partNote(part, rep.key);
        if (note) { notes.push(`${rep.label} here is ${note}.`); }
      }
    }
  ...
  /// The part's other representations on the bridge host, keyed the way the tracer keys them.
  /// The service holds ONE shape at a time, so these take turns with the exact sidecar and the
  /// tracer swaps them when the view changes.
  function otherPaths() {
    const part = state.part;
    const prefix = settings.prefix || '';
    const paths = {};
    for (const rep of REPRESENTATIONS) {
      if (rep.key === 'surface') { continue; }
      const relative = partPath(part, rep.key);
      paths[rep.key] = relative ? prefix + relative : null;
    }
    return paths;
  }
  ...
  function describeBridge() {
    ...
    // Every kind reports what it is made of, so the panel says which representation is resident
    // and how big it is without the page having to guess from the path.
    const shape = [];
    if (info.nSurfaces !== undefined) { shape.push(`${info.nSurfaces} surfaces`); }
    if (info.nCells !== undefined) { shape.push(`${info.nCells} cells, ${info.nHalfspaces} halfspaces`); }
    if (info.nFacets !== undefined) { shape.push(`${info.nFacets} facets`); }
    if (info.className) { shape.push(info.className); }
    if (info.booleanDepth) { shape.push(`${info.leaves} leaves at boolean depth ${info.booleanDepth}`); }
    if (info.singleThreaded) { shape.push('traced on ONE thread (TGeoBoolNode scratch state is not per-thread here)'); }
    const holding = tracer.bridgeLoaded ? ` holding ${tracer.bridgeLoaded}` : '';
    bridgeStatus.textContent = `bridge ready: ${info.kind || '?'}` +
      (shape.length ? `, ${shape.join(', ')}` : '') + holding;
  ...
  function describeView() {
    ...
    const notes = {
      ...
      mesh: 'The same camera, against the triangles of facets_*.bin in this page\'s JS port, with flat facet normals. The silhouette is a polygon.',
      diff: 'Red where one representation is hit and the other is not; otherwise a heatmap of the depth difference, saturating at one part in a thousand of the model size. On a part whose tessellation is EXACT -- every face a planar polygon -- there is nothing here to find: the two are the same solid, and a flat blue frame is the correct answer rather than a boring one.',
      ...
      originalBridge: 'The TGeoShape this part was made FROM, before it ever went to STEP -- the shape the detector geometry ships today -- traced by the real kernel through the bridge. This is the baseline: it is the input to the round trip, not one of its outputs, and it is what every other representation here has to beat. The ms/frame row is where that comparison stops being a table.',
      originalDiff: 'The exact surface solid against the shape the part was made from. Red here would be a round-trip defect: the two are supposed to be the same solid, and the converter\'s third acceptance test (checkKnownSource.py) is the instrument that says so on a point set rather than on a picture.',
      cellstreeBridge: 'The part\'s decomposed cells emitted as a plain TGeoCompositeShape. NOTHING ships this: it exists so the flat solid has something to be measured against, namely the tree the cascade refused to emit because those cells would make one wider than the 64-leaf routing threshold. Do not read it as what the converter produces.',
      cellstreeDiff: 'The exact surface solid against the cells as a plain composite, on the same rays. They are the same cells, so this should be flat and free of red.',
      flatcsgBridge: 'The part\'s FLAT-CSG solid -- a union of intersection-cells over signed implicit halfspaces, with a BVH over sub-boxes of those cells -- read from flatcsg_*.bin by the real kernel and traced through the bridge. It is what the part IS after decomposition, with nothing padded into a native primitive to make ROOT able to hold it. Same pixels as the exact view, a different representation, and the ms/frame row is where its measured speed-up stops being a table.',
      flatcsgDiff: 'The exact surface solid against the flat halfspace solid, on the same rays. A flat-CSG part converts at dV_sym exactly 0, so this picture should show no red at all and a flat blue heatmap.',
      meshBridge: 'The same triangles as the tessellation view, but answered by the real O2Tessellated in the kernel rather than by this page\'s JS port. The picture is the mesh\'s picture; the ms/frame row is the kernel\'s own mesh navigation cost, which is what the other three representations are competing with.',
  ...
  // The perf slots, in the order the table prints them: the JS port, then the same picture from
  // the kernel, then the kernel asked the same question of each of the part's other
  // representations. Every row is one engine's whole frame, so they are directly comparable.
  const PERF_ROWS = [
    ['local', 'local'],
    ['bridge exact', 'remote'],
    ['bridge Original TGeo', 'original'],
    ['bridge CSG', 'csg'],
    ['bridge cells-tree', 'cellstree'],
    ['bridge FlatCSG', 'flatcsg'],
    ['bridge mesh', 'meshKernel'],
  ];

  /// The other bridge subjects, quoted against the exact surfaces on the SAME kernel, so a ratio
  /// is representation against representation and not kernel against port.
  const PERF_OTHERS = [['Original TGeo', 'original'], ['CSG', 'csg'], ['cells-tree', 'cellstree'],
                       ['FlatCSG', 'flatcsg'], ['mesh', 'meshKernel']];

  function showPerf() {
    const rows = PERF_ROWS.map(([label, slot]) => [label, tracer.perf[slot]]);
    perfTable.innerHTML = '';
    ...
    for (const [key, entry] of rows) {
      ...
      const suffix = { 'bridge Original TGeo': ', the shape before the round trip',
                       'bridge CSG': ', the composite the converter ships',
                       'bridge cells-tree': ', the cells as a plain composite',
                       'bridge FlatCSG': ', flat halfspace solid',
                       'bridge mesh': ', O2Tessellated' }[key] || '';
      const cells = entry
        ? [`${entry.engine}${suffix}`, `${entry.ms}`, `${entry.rays}`, rate(entry)]
    ...
    if (a && b) {
      ...
      for (const [label, slot] of PERF_OTHERS) {
        const other = tracer.perf[slot];
        if (matches(b, other)) { bits.push(`bridge exact / bridge ${label} ${(b.ms / other.ms).toFixed(2)}x`); }
      }
  ...
  function showCounters(c) {
    ...
    if ((tracer.view === 'engine' || SUBJECT_DIFF_VIEWS.has(tracer.view)) && tracer.bridgeReady) {
      add('engine', `local vs ${tracer.remote.name}`);
    } else if (tracer.view === 'exactBridge' || REP_VIEWS.has(tracer.view)) {
      add('engine', tracer.remote ? tracer.remote.name : 'bridge (not connected)');
    } else { add('engine', tracer.local.name); }
    if (REP_VIEWS.has(tracer.view)) {
      const rep = representationFor(tracer.view);
      const role = { baseline: ' [the baseline, not a conversion output]',
                     comparison: ' [a comparison, shipped by nothing]' }[repRole(rep)] || '';
      add('representation', `${repLabel(rep)} on the bridge${role} -- ${REPRESENTATION_NOTE[rep] || ''}`);
    }
    add('pixels', `${c.total}`);
  ...
  tracer.onDone = (c) => {
    ...
    const others = PERF_OTHERS
      .map(([label, slot]) => [label, tracer.perf[slot]])
      .filter(([, entry]) => same(b, entry))
      .map(([label, entry]) => `  vs  bridge ${label} ${entry.ms} ms  (${(b.ms / entry.ms).toFixed(2)}x)`)
      .join('');
    const both = same(a, b)
      ? `\nlocal ${a.ms} ms  vs  bridge ${b.ms} ms  (${(b.ms / a.ms).toFixed(2)}x)` + others : '';
  ...
  viewSelect.addEventListener('change', () => {
    ...
    const needsBridge = tracer.view === 'engine' || tracer.view === 'exactBridge' || REP_VIEWS.has(tracer.view);
  ...
  async function autoConnect() {
    ...
    for (const prefix of candidates) {
      ...
      const saved = settings.prefix;
      settings.prefix = prefix;
      const result = await tracer.connectBridge(port, path, { paths: otherPaths() });
  ...
  document.getElementById('rt-timeboth').addEventListener('click', async () => {
    ...
    // Every other representation the part carries joins the matched set: same camera, same rays,
    // the same kernel -- only the representation differs. That IS the comparison this page exists
    // for, so it is one button and not one view switch per artefact.
    const views = ['exact', 'exactBridge',
                   ...Object.values(SUBJECT_VIEW).filter(v => tracer.viewReady(v))];
  ...
  document.getElementById('rt-connect').addEventListener('click', async () => {
    ...
    if (result.ok) {
      describeBridge();
      if (tracer.view === 'engine' || REP_VIEWS.has(tracer.view)) { scheduleRender(); }
    } else {
