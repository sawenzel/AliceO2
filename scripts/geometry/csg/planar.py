"""Is a part's tessellation the SAME SOLID as its exact surfaces, or an approximation of them?

A triangulation of a planar polygon **is** that polygon: three points span the plane exactly, and
the polygon's boundary is already straight lines, so nothing is lost. A triangulation of a
cylinder, cone, sphere or torus is not that surface. Nor is a triangulation of a *flat* face whose
boundary is an arc or a spline -- the face is exact and its outline is polygonised.

So the condition is narrow, and it is exactly the one `LoadSurfaceSolid` already applies when it
decides whether a plane record becomes a `PlanarPolygon` or a `CurvedPlanar` surface: **any** arc
or bspline edge anywhere on the face routes it through `AddCurvedPlanarSurface`. This module
answers the same question from the same bytes, so a part this calls exact is a part whose kernel
records are all `BVHSurfaceRecord::PlanarPolygon`.

Why the converter should care
-----------------------------
The cascade treats the tessellation as the fallback -- the thing a part gets when nothing better
worked. For a part whose every face is a planar polygon that framing is wrong twice over: the mesh
loses no accuracy, and it is small and fast. Measured on the nine all-planar parts that currently
ship `o2::base::O2FlatCSG`, the exact mesh beats the flat solid on every kernel -- median 2.0x on
`Contains`, 7.7x on `DistFromOutside`, 9.8x on `DistFromInside`, 3.5x on `Safety` -- at 72 to 324
facets. And on 30 random all-planar parts of ITS/TPC/TRD, 600 000 sample points, the mesh and the
exact surface solid disagreed about `Contains` zero times.

This module only *measures*; it does not route. `write_report` records the verdict per part so the
policy can be decided on evidence, and so the R6 census gets the number for free.
"""
...
def surface_census(path):
    ...
    Raises `ValueError` on a file this cannot read, so a caller can tell "not exact" from
    "could not tell", which are different answers and must not collapse into one.
    """
