    ...
    for lid, shape in def_shapes.items():
        ...
        (out_folder / f"csg_{suffix}.json").write_text(json.dumps(
            {"part": suffix, "lid": lid, "candidate": record["candidate"],
             "acceptance": record["acceptance"], "recogniser": record["recogniser"]}, indent=1))
        if record["accepted"]:
            if root_available:
...
def emit_csg_shape_cpp(lid, vol_display_name, shape_abspath, medium_var, sanitize_cpp_name):
    """geom.C branch for a CSG part: load the TGeoShape from its own file.
...
// The file holds exactly one object inheriting from TGeoShape under the key "shape", in cm and in
// the part's own local frame (scripts/geometry/Stream_G_AnyShape.md, and O2SolidHarness.h next to
// the C++ loader that reads the same convention).
