...
Provisional location: this belongs in a tooling/utils folder once one exists.
"""
...
def render(geofile, out, tiers=None, width=1100, height=800,
    ...
    # frame from the projection of every daughter's bbox corners, so an
    # outlying part shrinks the subject instead of being cropped
    # a stray part far from the rest of the model must not shrink the subject:
    # keep only daughters within 3x the median distance from the cluster median
    corners = []
