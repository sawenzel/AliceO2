...
This is the closure measure that survives contact with reality.  A hit-by-hit
comparison of charged tracks cannot: Geant draws from the RNG once per step, so a
geometry with even a slightly different boundary structure shifts every later
draw of a track, and the trajectories decorrelate.  Per-track seeding contains
that to one track; it does not remove it.  Measured on this test, only 10 of 198
primaries kept every hit within 1 cm, while the layer radii and the sensor
placement counts agreed exactly -- the divergence is the RNG, not the geometry.

A geantino has no RNG.  Integrating x/X0 and x/lambda along a fixed set of rays
through both geometries asks exactly the geometry question and answers it with a
number.  Directions come from a Fibonacci sphere, never an axis raster: three
axis beams are three directions however many rays are fired, and that trap has
already reported a real defect as clean twice in this project.
