...
def primary_map(rundir):
    ...
    A track index is not a shared name between two runs: the two stacks keep
    different secondaries, so track 9 of one is not track 9 of the other, and
    matching on it compares unrelated particles (measured: a median 23 cm
    residual between two geometries whose ITS layers agree to the millimetre).
    A primary IS shared -- same generator, same seed, same order -- so primaries
    are keyed by their ordinal within the event instead.
    """
    import ROOT
...
def compare_nearest(a, b, tol):
    ...
    Order-matching is the right test between two runs of the SAME geometry, and
    the wrong one between the native ITS and an external detector: the two do not
    define a hit the same way. ITS's own ProcessHits accumulates a hit per chip
    traversal; the external detector's built-in action records an entrance/exit
    pair per sensitive volume, and measured on this sample it produces 1.2 to 2.7
    times as many hits per layer. Comparing the n-th hit of each then compares
    different crossings and reports tens of centimetres of nonsense.

    So the question asked here is the one that is actually about geometry: does
    every hit the baseline recorded have a counterpart at the same place?
    """
