...
void TRDDPLTrapSimulatorTask::run(o2::framework::ProcessingContext& pc)
{
  ...
  std::vector<o2::trd::TriggerRecord> trackletTriggerRecords = triggerRecords; // = triggerRecords; // pc.outputs().make<std::vector<o2::trd::TriggerRecord>>(Output{"TRD", "TRKTRGRD", 0, Lifetime::Timeframe}, triggerRecords.begin() + 1, triggerRecords.end());
  ...
  std::vector<Tracklet> trapTrackletsAccum;// = pc.outputs().make<std::vector<Tracklet>>(Output{"TRD", "TRACKLETS", 0, Lifetime::Timeframe});
  int count = 0;
