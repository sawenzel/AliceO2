...
DataProcessorSpec getDigitWriterSpec(bool mctruth, o2::header::DataOrigin detOrig, o2::detectors::DetID detId)
{
  ...
  // delete tmp branches
  auto deleteTmpBranches = [](TTree* tree) {
    // we need to remove entries from 2 lists:
    auto leavelist = tree->GetListOfLeaves();
    for (auto entry : *leavelist) {
      LOG(INFO) << "PROBING LEAVE " << entry->GetName();
      if (TString(entry->GetName()).Contains("_TMP")) {
        LOG(INFO) << "REMOVING LEAVE";
        leavelist->Remove(entry);
      }
    }
    for (auto entry : *tree->GetListOfBranches()) {
      LOG(INFO) << "PROBING BRANCH " << entry->GetName();
      if (TString(entry->GetName()).Contains("_TMP")) {
        LOG(INFO) << "REMOVING BRANCH";
        auto br= static_cast<TBranch*>(entry);
        br->DropBaskets("all");
        br->DeleteBaskets("all");
        tree->GetListOfBranches()->Remove(entry);
      }
    }
  };

  // the callback to be set as hook for custom action when the writer is closed
  auto finishWriting = [&deleteTmpBranches](TFile* outputfile, TTree* outputtree) {
    deleteTmpBranches(outputtree);
  ...
  auto fillLabels = [detStr](TBranch& branch, std::vector<char> const& labelbuffer, DataRef const& /*ref*/) {
    ...
    // create new branch with correct type
