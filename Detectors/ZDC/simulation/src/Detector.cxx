...
void Detector::defineSensitiveVolumes()
{
  ...
  vol = gGeoManager->GetVolume("ZNEU");
  if(vol) AddSensitiveVolume(vol);
  else LOG(FATAL) << "can't find volume ZNEU" << FairLogger::endl;

  vol = gGeoManager->GetVolume("ZPRO");
  if(vol) AddSensitiveVolume(vol);
  else LOG(FATAL) << "can't find volume ZPRO" << FairLogger::endl;
}
...
Bool_t Detector::ProcessHits(FairVolume* v)
{
  ...
  float x[3]={0.,0.,0.}, xDet[3]={0.,0.,0.}, p[3]={0.,0.,0.}, energy=0.;
  fMC->TrackPosition(x);
  ...
  if(volname.Contains("ZN")){
     ...
     if(mZDCdetID[0]==1){
       for(int i=0; i<3; i++) xdet[i] = x[i]-ZNAPOSITION[i];
       if(xdet[0]<=0.){
       ...
       for(int i=0; i<3; i++) xdet[i] = x[i]-ZNCPOSITION[i];
       if(xdet[0]<=0.){
         if(xdet[1]<=0.) mZDCdetID[1] = 1;
         else  mZDCdetID[1] = 3;
       }
     ...
     //
    if(mZDCdetID[0]==2){
       for(int i=0; i<3; i++) xdet[i] = x[i]-ZPAPOSITION[i];
       float xTow = xdet[0]/(ZPDIMENSION[0]/2.);
       for(int i=1; i<=4; i++){
       ...
       for(int i=0; i<3; i++) xdet[i] = x[i]-ZPCPOSITION[i];
       float xTow = xdet[0]/(ZPDIMENSION[0]/2.);
       for(int i=1; i<=4; i++){
          if(xTow>=(i-3) && xTow<(i-2)){
            mZDCdetID[1] = i;
     ...
     mZDCdetID[0]=3;
     for(int i=0; i<3; i++) xdet[i] = x[i]-ZEMPOSITION[i];
  ...
  if((fMC->Edep)>1e-04) CalculateTableIndexes(ibeta, iangle, iradius);
  ...
  Hit* p = addHit(curTrackn, pdgCode, parent, sFlag, p[3], mZDCdetID, x, p, mTrackTof, mXImpact, mTotDepEnergy, mTotLight);
...
void Detector::CreateMaterials()
{
  ...
  Int_t inofld = 0;
  Int_t ifld = 3;  ????CHECK!!!! secondo me va -1!!!!!
  Float_t nofieldm = 0.;
  ...
  Medium(kVoiNoField, "VoidNoField$", 8, notactiveMed, inofld, nofieldm, tmaxnofd, stemax, deemax, epsil, stmin);
  Medium(kVoiwField, "VoidwField$", 8, notactiveMed, ifld, nofieldm, tmaxnofd, stemax, deemax, epsil, stmin);
...
void Detector::CreateAsideBeamline()
{
  ...
  boxpar[1] = 2.2/2.;
  boxpar[2] = 100./2.;
  TVirtualMC::GetMC()->Gsvolu("Q11T", "BOX ", getMediumID(kVoiNoField), boxpar, 3);
  ...
  TVirtualMC::GetMC()->Gspos("Q08T", 1, "ZDCA", 0., fTCDDAperturePos, -100.+zA, 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("Q10T", 1, "ZDCA", 0., -fTCDDApertureNeg, -100.+zA, 0, "ONLY");
  ...
  tubpar[1] = 54.6/2.;
  tubpar[2] = 540.0/2.;
  TVirtualMC::GetMC()->Gsvolu("Q13TM", "TUBE", getMediumID(kVoidNoFiels), tubpar, 3);
...
void Detector::CreateCsideBeamline()
{
  ...
  if(Geometry::TCLIAAPERTURE<3.5){
    boxpar[0] = 5.4/2.;
    boxpar[1] = (3.5-fVCollSideCAperture-fVCollSideCCentreY-0.7)/2.; // FIX IT!!!!!!!!
    if(boxpar[1]<0.) boxpar[1]=0.;
    boxpar[2] = 124.4/2.;
    TVirtualMC::GetMC()->Gsvolu("QCVC" , "BOX ", getMediumID(kGraphite), boxpar, 3);
    TVirtualMC::GetMC()->Gspos("QCVC", 1, "QE02", -boxpar[0],  fVCollSideCAperture+fVCollSideCCentreY+boxpar[1], -totLength1/2.+160.8+78.+148./2., 0, "ONLY");  FIX IT!!!!!!!!
    TVirtualMC::GetMC()->Gspos("QCVC", 2, "QE02", -boxpar[0], -fVCollSideCApertureNeg+fVCollSideCCentreY-boxpar[1], -totLength1/2.+160.8+78.+148./2., 0, "ONLY");  FIX IT!!!!!!!!
  }
  ...
  // Volume: QCLext
  TGeoMedium *medZDCFe = gGeoManager->GetMedium("ZDC_Iron");
  TGeoVolume *pQCLext = new TGeoVolume("QCLext", pOutTrousersC, getMediumID(kVoidNoField));
  //
...
void Detector::CreateSupports()
{
  ...
  TVirtualMC::GetMC()->Gsvolu("ZNBASE","BOX ", getMediumID(kAl), Geometry::ZNSUPPORTBASE, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZNSCH","BOX ", getMediumID(kAl), Geometry::ZNSUPPORTSCINTILH, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZNSCV","BOX ", getMediumID(kAl), Geometry::ZNSUPPORTSCINTILV, 3);
  ...
  // Top & bottom walls
  TVirtualMC::GetMC()->Gsvolu("ZNBH","BOX ", getMediumID(kAl), Geometry::ZNSUPPORTWALLSUD, 3);
  TVirtualMC::GetMC()->Gspos("ZNBH", 1, "cave", Geometry::ZNAPOSITION[0]+Geometry::ZNSUPPORTBASEPOS[0], Geometry::ZNAPOSITION[1]+Geometry::ZNSUPPORTBASEPOS[1]+ZNSUPPORTSCINTILV[1]+Geometry::ZNSUPPORTWALLSUD[1], Geometry::ZNCPOSITION[2]-Geometry::ZNSUPPORTWALLSUD[2], 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZNBH", 2, "cave", Geometry::ZNAPOSITION[0]+Geometry::ZNSUPPORTBASEPOS[0], Geometry::ZNAPOSITION[1]+Geometry::ZNSUPPORTBASEPOS[1]+ZNSUPPORTSCINTILV[1]+Geometry::ZNSUPPORTWALLSUD[1]+2.*ZNDIMENSION[1], Geometry::ZNCPOSITION[2]-Geometry::ZNSUPPORTWALLSUD[2], 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZNBH", 3, "cave", Geometry::ZNAPOSITION[0]+Geometry::ZNSUPPORTBASEPOS[0], Geometry::ZNAPOSITION[1]+Geometry::ZNSUPPORTBASEPOS[1]+ZNSUPPORTSCINTILV[1]+Geometry::ZNSUPPORTWALLSUD[1], Geometry::ZNAPOSITION[2]+Geometry::ZNSUPPORTWALLSUD[2], 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZNBH", 4, "cave", Geometry::ZNAPOSITION[0]+Geometry::ZNSUPPORTBASEPOS[0], Geometry::ZNAPOSITION[1]+Geometry::ZNSUPPORTBASEPOS[1]+ZNSUPPORTSCINTILV[1]+Geometry::ZNSUPPORTWALLSUD[1]+2.*ZNDIMENSION[1], Geometry::ZNAPOSITION[2]+Geometry::ZNSUPPORTWALLSUD[2], 0, "ONLY");

  // Side walls
  TVirtualMC::GetMC()->Gsvolu("ZNBS","BOX ", getMediumID(kAl), Geometry::ZNSUPPORTWALLSLAT, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZPBASE1","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTBASE1, 3);
  ...
  TVirtualMC::GetMC()->Gspos("ZPBASE1", 2, "cave", Geometry::ZPAPOSITION[0]+Geometry::ZPSUPPORTBASEPOS[0],
  ...
  TVirtualMC::GetMC()->Gsvolu("ZPFOOT","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTBASE3, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZPBASE2","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTBASE2, 3);
  ...
  // Bottom walls
  TVirtualMC::GetMC()->Gsvolu("ZPBB","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTWALLBOTTOM, 3);
  ...
  // Top walls
  TVirtualMC::GetMC()->Gsvolu("ZPBT","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTWALLUP, 3);
  ...
  // Side walls
  TVirtualMC::GetMC()->Gsvolu("ZPBS","BOX ", getMediumID(kAl), Geometry::ZPSUPPORTWALLSLAT, 3);
  ...
  // Bridge
  TVirtualMC::GetMC()->Gsvolu("ZESH","BOX ", getMediumID(kAl), Geometry::ZEMSUPPORTU1, 3);
  Float_t ybridge = Geometry::ZEMPOSITION[1]-Geometry::ZEMDIMENSION[1]-2.*2.*Geometry::ZEMSUPPORTBOX[3+1]-5.-Geometry::ZEMSUPPORTU1[1];
  ...
  //
  TVirtualMC::GetMC()->Gsvolu("ZESV","BOX ", getMediumID(kAl), Geometry::ZEMSUPPORTU2, 3);
  TVirtualMC::GetMC()->Gspos("ZESV", 1, "cave", Geometry::ZEMPOSITION[0]-Geometry::ZEMSUPPORTBOX[0]+Geometry::ZEMSUPPORTU2[0], ybox-Geometry::ZEMSUPPORTBOX[1]-Geometry::ZEMSUPPORTU2[1], zbox, 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZESV", 2, "cave", Geometry::ZEMPOSITION[0]+Geometry::ZEMSUPPORTBOX[0]-Geometry::ZEMSUPPORTU2[0], ybox-Geometry::ZEMSUPPORTBOX[1]-Geometry::ZEMSUPPORTU2[1], zbox, 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZESV", 3, "cave", -(Geometry::ZEMPOSITION[0]-Geometry::ZEMSUPPORTBOX[0]+Geometry::ZEMSUPPORTU2[0]), ybox-Geometry::ZEMSUPPORTBOX[1]-Geometry::ZEMSUPPORTU2[1], zbox, 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZESV", 4, "cave", -(Geometry::ZEMPOSITION[0]+Geometry::ZEMSUPPORTBOX[0]-Geometry::ZEMSUPPORTU2[0]), ybox-Geometry::ZEMSUPPORTBOX[1]-Geometry::ZEMSUPPORTU2[1], zbox, 0, "ONLY");

  // Table
  TVirtualMC::GetMC()->Gsvolu("ZETA","BOX ", getMediumID(kkAl), Geometry::ZEMSUPPORTTABLE, 3);
  Float_t ytable = ybridge-Geometry::ZEMSUPPORTU1[1]-Geometry::ZEMSUPPORTTABLE[1];
  ...
  //Screens around ZEM
  TVirtualMC::GetMC()->Gsvolu("ZEFL","BOX ", getMediumID(kAl), Geometry::ZEMSUPPORTU3, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZELA", "PARA", getMediumID(kAl), Geometry::ZEMSUPPORTU4, 6);
  TVirtualMC::GetMC()->Gspos("ZELA", 1, "cave", Geometry::ZEMPOSITION[0]-Geometry::ZEMDIMENSION[2]-Geometry::ZEMSUPPORT4[2], fPosZEM[1], Geometry::ZEMPOSITION[2]+Geometry::ZEMSUPPORT4[0], irotzem1, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZELA", 2, "cave", Geometry::ZEMPOSITION[0]+Geometry::ZEMDIMENSION[2]+Geometry::ZEMSUPPORT4[2], fPosZEM[1], Geometry::ZEMPOSITION[2]+Geometry::ZEMSUPPORT4[0], irotzem1, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZELA", 3, "cave", -(Geometry::ZEMPOSITION[0]-Geometry::ZEMDIMENSION[2]-Geometry::ZEMSUPPORT4[2]), fPosZEM[1], Geometry::ZEMPOSITION[2]+Geometry::ZEMSUPPORT4[0], irotzem1, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZELA", 4, "cave", -(Geometry::ZEMPOSITION[0]+Geometry::ZEMDIMENSION[2]+Geometry::ZEMSUPPORT4[2]), fPosZEM[1], Geometry::ZEMPOSITION[2]+Geometry::ZEMSUPPORT4[0], irotzem1, "ONLY");

  // Containers for ZEM calorimeters
  TVirtualMC::GetMC()->Gsvolu("ZEW1","BOX ", getMediumID(kAl), Geometry::ZEMWALLH, 3);
  TVirtualMC::GetMC()->Gsvolu("ZEW2","BOX ", getMediumID(kAl), Geometry::ZEMWALLVFWD, 3);
  TVirtualMC::GetMC()->Gsvolu("ZEW3","BOX ", getMediumID(kAl), Geometry::ZEMWALLBKW, 3);
  TVirtualMC::GetMC()->Gsvolu("ZEW4","BOX ", getMediumID(kFe), Geometry::ZEMWALLSIDE, 3);
  //
  Float_t yh1 = Geometry::ZEMPOSITION[1]-Geometry::ZEMDIMENSION[1]-2*Geometry::ZEMSUPPORTU3[1]-Geometry::ZEMWALLH[1];
  ...
  //
  Float_t xl1= Geometry::ZEMPOSITION[0]-Geometry::ZEMDIMENSION[2]-2.*Geometry::ZEMSUPPORTU4[2]-Geometry::ZEMWALLSIDE[0];
  Float_t xl2= Geometry::ZEMPOSITION[0]+Geometry::ZEMDIMENSION[2]+2.*Geometry::ZEMSUPPORTU4[2]+Geometry::ZEMWALLSIDE[0];
...
void Detector::CreateDetectors()
{
  ...
  TVirtualMC::GetMC()->Gsvolu("ZNEU", "BOX ", getMediumID(kWalloy),  Geometry::ZNDIMENSION, 3); // Passive material
  TVirtualMC::GetMC()->Gsvolu("ZNF1", "TUBE", getMediumID(kSiO2pmc), Geometry::ZNFIBRE, 3); // Active material
  TVirtualMC::GetMC()->Gsvolu("ZNF2", "TUBE", getMediumID(kSiO2pmq), Geometry::ZNFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZNF3", "TUBE", getMediumID(kSiO2pmq), Geometry::ZNFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZNF4", "TUBE", getMediumID(kSiO2pmc), Geometry::ZNFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZNG1", "BOX ", getMediumID(kAir), Geometry::ZNGROOVES, 3); // Empty grooves
  TVirtualMC::GetMC()->Gsvolu("ZNG2", "BOX ", getMediumID(kAir), Geometry::ZNGROOVES, 3);
  TVirtualMC::GetMC()->Gsvolu("ZNG3", "BOX ", getMediumID(kAir), Geometry::ZNGROOVES, 3);
  TVirtualMC::GetMC()->Gsvolu("ZNG4", "BOX ", getMediumID(kAir), Geometry::ZNGROOVES, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZPRO", "BOX ", getMediumID(kCuZN), Geometry::ZPDIMENSION, 3); // Passive material
  TVirtualMC::GetMC()->Gsvolu("ZPF1", "TUBE", getMediumID(kSiO2pmc), Geometry::ZPFIBRE, 3); // Active material
  TVirtualMC::GetMC()->Gsvolu("ZPF2", "TUBE", getMediumID(kSiO2pmq), Geometry::ZPFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZPF3", "TUBE", getMediumID(kSiO2pmq), Geometry::ZPFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZPF4", "TUBE", getMediumID(kSiO2pmc), Geometry::ZPFIBRE, 3);
  TVirtualMC::GetMC()->Gsvolu("ZPG1", "BOX ", getMediumID(kAir), Geometry::ZPGROOVES, 3);  // Empty grooves
  TVirtualMC::GetMC()->Gsvolu("ZPG2", "BOX ", getMediumID(kAir), Geometry::ZPGROOVES, 3);
  TVirtualMC::GetMC()->Gsvolu("ZPG3", "BOX ", getMediumID(kAir), Geometry::ZPGROOVES, 3);
  TVirtualMC::GetMC()->Gsvolu("ZPG4", "BOX ", getMediumID(kAir), Geometry::ZPGROOVES, 3);
  ...
  TVirtualMC::GetMC()->Gsvolu("ZEM ", "PARA", getMediumID(kVoidNoField), Geometry::ZEMDIMENSION, 6);
  TVirtualMC::GetMC()->Gsvolu("ZEMF", "TUBE", getMediumID(kSiO2pmc), Geometry::ZEMFIBRE, 3); 	// Active material
  TVirtualMC::GetMC()->Gsdvn("ZETR", "ZEM ", Geometry::ZEMDIVISION[2], 1); 	     	// Tranches

  TVirtualMC::GetMC()->Gsvolu("ZEL0", "PARA", getMediumID(kPb), Geometry::ZEMPBSLICE, 6); 	   // Lead slices
  TVirtualMC::GetMC()->Gsvolu("ZEL1", "PARA", getMediumID(kPb), Geometry::ZEMPBSLICE, 6);
  TVirtualMC::GetMC()->Gsvolu("ZEL2", "PARA", getMediumID(kPb), Geometry::ZEMPBSLICE, 6);

  // --- Position the lead slices in the tranche
  TVirtualMC::GetMC()->Gspos("ZEL0", 1, "ZETR", -Geometry::ZEMVOIDLAYER[0]+Geometry::ZEMPBSLICE, 0., 0., 0, "ONLY");
  TVirtualMC::GetMC()->Gspos("ZEL1", 1, "ZETR", Geometry::ZEMPBSLICE, 0., 0., 0, "ONLY");

  // --- Vacuum zone (to be filled with fibres)
  TVirtualMC::GetMC()->Gsvolu("ZEV0", "PARA", getMediumID(kVoidNoField), Geometry::ZEMVOIDLAYER, 6);
  TVirtualMC::GetMC()->Gsvolu("ZEV1", "PARA", getMediumID(kVoidNoField), Geometry::ZEMVOIDLAYER, 6);
  ...
  TVirtualMC::GetMC()->Gspos("ZEV1", 1,"ZETR", -Geometry::ZEMVOIDLAYER[0]+-Geometry::ZEMVOIDLAYER[0]+Geometry::ZEMPBSLICE, 0., 0., 0, "ONLY");
